...
export default function TotalCostCard({
  ...
  const legal = Math.round(50 + price * 0.00015)
  const moving = 150
  const legalCapped = Math.min(legal, 200)
  ...
  const rows = [
    { label: '매입가', value: price, sub: '' },
    {
      label: '취득세',
      value: tax.final,
      sub: isFirstBuyer && tax.discount > 0 ? `생애최초 감면 −${formatPrice(tax.discount)}` : '',
    },
      ...
      <div className="divide-y divide-gray-50">
        {rows.map(r => (
          <div key={r.label} className="flex items-center justify-between py-1.5">
            ...
            <span className="text-sm font-medium text-gray-800">{formatPrice(r.value)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="border-t-2 border-gray-200 pt-2.5 space-y-2">
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-sm text-gray-400">
            <span>내 자기자본</span>
            <span>{formatPrice(selfFunds)}</span>
          </div>
          {effectiveLoan > 0 && (
            <div className="flex justify-between text-sm text-gray-400">
              <span>대출 가능액 (추정)</span>
              <span>{formatPrice(effectiveLoan)}</span>
            </div>
          )}
        </div>
        <div className={`flex items-center justify-between text-sm font-bold pt-2 border-t border-dashed ${isShortfall ? 'text-red-600' : 'text-green-600'}`}>
          ...
          <span>
            {isShortfall
              ? `${formatPrice(Math.abs(gap))} 부족`
              : `${formatPrice(Math.abs(gap))} 여유`}
          </span>
        </div>
